...
def formatDocument(input, output):
    ...
    for para in document.paragraphs:
        ...
        if (para_text != ""):
            ...
            not_header_words_present = [ele for ele in not_header_words if search(r"(?i)(?<!\S)" + escape(ele) + r"[\.:]{0,1}" + r"(?!\S)", para_text)]   

            # Check for Heading 1 text (starting with header_1_names_list or numeric value and max 75 characters and no 'not_header_words' present and no . at the end)
            if (((len(para_text) <= CHAPTER_MAX_LENGTH) or (para.style.name == heading_style.name)) and (not not_header_words_present) and para_text[-1] != '.'):
